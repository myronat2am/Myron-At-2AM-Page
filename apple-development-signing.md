...
在比較之前，我們先理解這三種模式的核心運作邏輯：

### 1. App Store (官方通路)
*   **機制**：開發者將 App 上傳至 App Store Connect → Apple 進行人工審核 → 通過後，Apple 會用官方的憑證重新對 App 進行加密簽名 → 上架供全球用戶下載。
*   **特點**：這是唯一「正規」的途徑。使用者下載到的 App 是由 Apple 擔保的。開發者無法得知具體是哪位使用者下載了 App，隱私性最高。

### 2. 企業簽 (In-House Distribution)
*   **機制**：Apple 發給大型企業 專用的「企業開發者帳號」($299/年)。企業使用此憑證對 App 簽名 → 放在自家伺服器或 MDM 系統 → 員工下載後，需在「設定」中手動點擊「信任」該企業描述檔。
*   **特點**：
    *   **無需審核**：完全繞過 Apple 審核，功能無限制（可使用私有 API）。
    *   **權限巨大**：一旦憑證被 Apple 發現用於分發給非員工（例如博弈、色情 App），會直接**撤銷 (Revoke)** 憑證，導致全球所有已安裝該 App 的設備瞬間閃退。

### 3. 超級簽 (Super Signature)
*   **機制**：這其實是利用了 Apple 給開發者進行「真機測試」的 **Ad-Hoc 通道**。
    *   正常開發流程：開發者把測試手機的 UDID 加到帳號 → 打包 App → 測試者安裝。
    *   **超級簽的操作**：透過自動化腳本，當使用者點擊下載時，系統自動獲取其 UDID → 自動註冊到一個閒置的個人開發者帳號 ($99/年) 下 → 自動申請描述檔並重簽 → 回傳給使用者安裝。
*   **特點**：
    *   把每一個付費下載的陌生人，都偽裝成公司的「內部測試人員」。
    *   因為每個帳號只能添加 100 台設備，所以成本極高（需大量購買開發者帳號），且一旦帳號被封，這 100 位用戶就無法更新或使用。
